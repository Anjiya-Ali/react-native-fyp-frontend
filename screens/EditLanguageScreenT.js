...
const EditLanguageScreenT = ({ route }) => {

  const context = useContext(teacherProfileContext);
  const { deleteLanguage } = context;
  const { additionalData } = route.params;

  const [localLanguages, setLocalLanguages] = useState(additionalData);
  ...
  const handleDelete = async () => {
    await deleteLanguage(requestId);
    setLocalLanguages(localLanguages.filter((language) => language.id !== requestId));
    setShowRejectModal(false);
  };

  const handleUpdateLanguage = async (name, level, id) => {
    navigation.navigate("UpdateSingleLanguageScreenT", {
      additionalData: { name, level, id },
    })
  };
    ...
    >
      ...
      <View style={styles.sectionContainer}>
        <View style={styles.sectionIconsContainer}>
          <Text style={styles.sectionTitle}>Edit Languages</Text>
        </View>
        {localLanguages.map((language, index) => (
          <View key={index}>
            ...
            <View style={styles.sectionIconsContainer}>
              <TouchableOpacity onPress={() => handleUpdateLanguage(language.name, language.level, language.id)}>
                <Image
                  style={styles.icon}
                  resizeMode="cover"
              ...
              { localLanguages.length > 1 &&
                (
                  <TouchableOpacity onPress={() => handleDeleteLanguage(language.id)}>
                  <Image
                    style={styles.icon}
                    resizeMode="cover"
...
export default EditLanguageScreenT;
