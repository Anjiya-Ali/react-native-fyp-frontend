  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        ...
        setAllPendingConnections(pendingConnections || []);
        setLocalRequests(pendingConnections || []);
        setUserPrivilege(privilege);
      };

      fetchData();
    }, [getPendingConnections, pendingConnections, privilege])
  );
      ...
      >
        <View style={[styles.headerPosition, { position: "relative" }]}>
          ...
          <Text style={styles.userName1}>
            You have {allPendingConnections.length} requests
          </Text>
        </View>
