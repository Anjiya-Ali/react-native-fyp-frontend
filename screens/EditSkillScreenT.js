...
const EditSKillScreenT = ({ route }) => {
  ...
  const handleDelete = async () => {
    await deleteSkill(requestId);
    setLocalSkills(localSkills.filter((skill) => skill.id !== requestId));
            ...
            <View style={styles.sectionIconsContainer}>
              { localSkills.length > 1 &&
                (
                  ...
                  <Image
                    style={[styles.icon, { justifyContent: 'flex-start' }]}
                    resizeMode="cover"
...
export default EditSKillScreenT;