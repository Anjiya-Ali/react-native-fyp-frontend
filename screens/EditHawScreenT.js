...
const EditHawScreenT = ({ route }) => {

  const context = useContext(teacherProfileContext);
  const { deleteHaw } = context;
  const { additionalData } = route.params;

  const [localHaw, setLocalHaw] = useState(additionalData);
  ...
  const handleDeleteHaw = async (id) => {
    setRequestId(id)
    setShowRejectModal(true);
  };

  const handleDelete = async () => {
    await deleteHaw(requestId);
    setLocalHaw(localHaw.filter((haw) => haw.id !== requestId));
    setShowRejectModal(false);
  };

  const handleUpdateHaw = async (title, issuer, issue_date, id) => {
    navigation.navigate("UpdateSingleHawScreenT", {
      additionalData: { title, issuer, issue_date, id },
    })
  };
    ...
    >
      ...
      <View style={styles.sectionContainer}>
        <View style={styles.sectionIconsContainer}>
          <Text style={styles.sectionTitle}>Edit Honors and Awards</Text>
        </View>
        {localHaw.map((haw, index) => (
          <View key={index}>
            ...
            <View style={styles.sectionIconsContainer}>
              <TouchableOpacity onPress={() => handleUpdateHaw(haw.title, haw.issuer, haw.issue_date, haw.id)}>
                <Image
                  style={styles.icon}
                  resizeMode="cover"
              ...
              { localHaw.length > 1 &&
                (
                  <TouchableOpacity onPress={() => handleDeleteHaw(haw.id)}>
                  <Image
                    style={styles.icon}
                    resizeMode="cover"
