...
const UpdateSingleProjectScreenT = ({ route }) => {
  ...
  const [projectTitle, setProjectTitle] = useState(additionalData.title);
  const [projectDescription, setProjectDescription] = useState(additionalData.description);
  const [projectLink, setProjectLink] = useState(additionalData.link);
  ...
  const handleUpdateProject = async (id) => {
    try {
        setError(null);
        if (projectLink.trim() === "" || projectTitle.trim() === "" || projectDescription.trim() === "" || value === null || value1 === null) {
            ...
            if(parseInt(value) > parseInt(value1)){
                ...
                await editProject(projectTitle, projectDescription, parseInt(value), parseInt(value1), projectLink, id);
  ...
  return (
    <ScrollView style={{ flex: 1, flexDirection: flexD, backgroundColor: "#d9d9d9" }}>
      ...
      <View style={styles.container}>
        <Text style={styles.header}>Update Project</Text>
        {error && (
        ...
        <View style={styles.sectionContainer}>
          ...
          <TextInput defaultValue={projectTitle} placeholderTextColor="black"value={projectTitle} onChangeText={(text) => setProjectTitle(text)} multiline={true} style = {styles.input}/>
        </View>
        <View style={styles.sectionContainer}>
          <Text style={styles.sectionTitle}>Description</Text>
          <TextInput defaultValue={projectDescription} placeholderTextColor="black"value={projectDescription} onChangeText={(text) => setProjectDescription(text)} multiline={true} style = {styles.input}/>
        </View>
        <View style={styles.sectionContainer}>
        ...
        <View style={styles.sectionContainer}>
          ...
          <TextInput defaultValue={projectLink} placeholderTextColor="black"value={projectLink} onChangeText={(text) => setProjectLink(text)} multiline={true} style = {styles.input}/>
        </View>
        <TouchableOpacity
          onPress={() => handleUpdateProject(additionalData.id)}
...
export default UpdateSingleProjectScreenT;