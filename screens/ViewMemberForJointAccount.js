...
const ViewMemberForJointAccount = ({route}) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        await getConnections(additionalData);
        setAllConnections(connections || []);
        setLocalRequests(connections || []);
        await getCourseById(additionalData);
        setLocalCourseName(courseName);
      };

      fetchData();
    }, [getConnections, connections, getCourseById, courseName])
  );
  ...
  const handleInviteConnection = async (connectionId, name) => {
    navigation.navigate("WriteMessageForJointAccount", {
        additionalData: {courseId : additionalData, teacherId: connectionId, fullName: name, courseName},
    });
  };
            ...
            {localRequests.map((connection, index) => (
              <View key={index}>
                <View style={styles.sectionContainer}>
                  ...
                  <TouchableOpacity onPress={() => handleInviteConnection(connection.id, connection.name)}>
