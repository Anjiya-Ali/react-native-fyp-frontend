...
const EditCertificationScreenT = ({ route }) => {

  const context = useContext(teacherProfileContext);
  const { deleteCertification } = context;
  const { additionalData } = route.params;

  const [localCertification, setLocalCertification] = useState(additionalData);
  ...
  const handleDeleteCertification = async (certificationId) => {
    setRequestId(certificationId)
    setShowRejectModal(true);
  };

  const handleDelete = async () => {
    await deleteCertification(requestId);
    setLocalCertification(localCertification.filter((certification) => certification.id !== requestId));
    setShowRejectModal(false);
  };


  const handleUpdateCertification = async (title, issuer, link, id) => {
    navigation.navigate("UpdateSingleCertificationScreenT", {
      additionalData: { title, issuer, link, id },
    })
  };
    ...
    >
      ...
      <View style={styles.sectionContainer}>
        <View style={styles.sectionIconsContainer}>
          <Text style={styles.sectionTitle}>Edit Certifications</Text>
        </View>
        {localCertification.map((certificate, index) => (
          <View key={index}>
            ...
            <View style={styles.sectionIconsContainer}>
              <TouchableOpacity onPress={() => handleUpdateCertification(certificate.title, certificate.issuer, certificate.link, certificate.id)}>
                <Image
                  style={styles.icon}
                  resizeMode="cover"
              ...
              { localCertification.length > 1 &&
                (
                  <TouchableOpacity onPress={() => handleDeleteCertification(certificate.id)}>
                  <Image
                    style={styles.icon}
                    resizeMode="cover"
...
export default EditCertificationScreenT;